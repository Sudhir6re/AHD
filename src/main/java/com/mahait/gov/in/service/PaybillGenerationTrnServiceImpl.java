	...
	public Long savePaybillHeadMpg(PaybillHeadMpgModel paybillHeadMpgModel) {/*
		...
		for (MstEmployeeEntity mstEmployeeEntity2 : mstEmployeeEntity) {
			...
			for (Object[] object12 : object) {
				...
				case 3:
					...
					// CREDIT_SOC
							...
							CommonConstants.PAYBILLDETAILS.COMMONCODE_COMPONENT_CREDIT_SOC)) {
						EmployeeAllowDeducComponentAmtEntity employeeAllowDeducComponentAmtEntity = mstEmployeeService
								.findGRPComponentsData(mstEmployeeEntity2.getSevaarthId(),
										CommonConstants.PAYBILLDETAILS.COMMONCODE_COMPONENT_amrawati_dist_engg_credit_soc_Code);
						
						if (employeeAllowDeducComponentAmtEntity != null
								&& allowDeducCode == CommonConstants.PAYBILLDETAILS.COMMONCODE_COMPONENT_CREDIT_SOC_CODE) {
							
							paybillGenerationTrnDetails.setCreditSoc(
									(double) Math.round(employeeAllowDeducComponentAmtEntity.getNetAmt()));
							CreditSoc = (double) (Math
									.round(employeeAllowDeducComponentAmtEntity.getNetAmt()));
						} else {
							paybillGenerationTrnDetails.setCreditSoc((double) 0);
							CreditSoc += 0;
						}
					}
					// CON_STORE
						...
						if (employeeAllowDeducComponentAmtEntity != null
								&& allowDeducCode == CommonConstants.PAYBILLDETAILS.COMMONCODE_COMPONENT_CON_STORE_CODE) {
							
							paybillGenerationTrnDetails.setConStore(
									(double) Math.round(employeeAllowDeducComponentAmtEntity.getNetAmt()));
							conStore = (double) (Math
									.round(employeeAllowDeducComponentAmtEntity.getNetAmt()));
						} else {
							paybillGenerationTrnDetails.setConStore((double) 0);
							conStore += 0;
						}
					} 
					// MTR_CO_OP_SOC
						...
						if (employeeAllowDeducComponentAmtEntity != null
								&& allowDeducCode == CommonConstants.PAYBILLDETAILS.COMMONCODE_COMPONENT_MTR_CO_OP_SOC_CODE) {
							
							paybillGenerationTrnDetails.setMtrCoOpSoc(
									(double) Math.round(employeeAllowDeducComponentAmtEntity.getNetAmt()));
							mtrCoSoc = (double) (Math
									.round(employeeAllowDeducComponentAmtEntity.getNetAmt()));
						} else {
							paybillGenerationTrnDetails.setMtrCoOpSoc((double) 0);
							mtrCoSoc += 0;
						}
					}
					// MIS
						...
						if (employeeAllowDeducComponentAmtEntity != null
								&& allowDeducCode == CommonConstants.PAYBILLDETAILS.COMMONCODE_COMPONENT_MIS_CODE) {
							
							paybillGenerationTrnDetails.setMis(
									(double) Math.round(employeeAllowDeducComponentAmtEntity.getNetAmt()));
							mis = (double) (Math
									.round(employeeAllowDeducComponentAmtEntity.getNetAmt()));
						} else {
							paybillGenerationTrnDetails.setMis((double) 0);
							mis += 0;
						}
					}
			...
			double dedByOthr = CA + FA + HouseAdv + vehAdv + gpfAdvance + alliedSoc + mantralayaSoci + chiplunSoc
					...
					+ motorCycleAdvIntAmt + bhanZillaParWaPanchayatSamitiSanstha +mtrCoSoc+conStore+mis;
			paybillGenerationTrnDetails.setDedAdjOtr((double) Math.round(dedByOthr));
	...
	*/
	...
	public Long savePaybillHeadMpgNew(PaybillHeadMpgModel paybillHeadMpgModel) {
		...
		// Start Travels Allowances for 6PC
					...
					ta=paybillHeadMpgRepo.fetchtaDtls(basic,startDate,payCommission,mstEmployeeEntity2.getGradePay(),cityClass,mstEmployeeEntity2.getPhysicallyHandicapped());
		...
		// Start Travels Allowances for 7PC
						...
						&& str != CommonConstants.PAYBILLDETAILS.COMMONCODE_VALUE_NULL) {
					Long gradelevel = mstEmployeeEntity2.getSevenPcLevel();
					ta=paybillHeadMpgRepo.fetchtaDtls(basic,startDate,payCommission,gradelevel,cityClass,mstEmployeeEntity2.getPhysicallyHandicapped());
					ta = paybillGenerationTrnDetails.getTa();
				}
			/* HRA component */
//				
				...
				// Professional Tax//
					...
					if (paybillHeadMpgModel.getPaybillMonth() == 2) {
						...
						if (basic > 0) {
							if (basic <= CommonConstants.PAYBILLDETAILS.COMMONCODE_PT_AMOUNT_LESS_THAN_4999)
								pt = CommonConstants.PAYBILLDETAILS.COMMONCODE_PT_AMOUNT_0;
							else if (basic > CommonConstants.PAYBILLDETAILS.COMMONCODE_PT_AMOUNT_LESS_THAN_4999
									&& basic <= CommonConstants.PAYBILLDETAILS.COMMONCODE_PT_AMOUNT_GREATER_THAN_9999)
								pt = 175d;
				...
				// Group_Acc_policy
				else if (str.equalsIgnoreCase(CommonConstants.PAYBILLDETAILS.COMMONCODE_COMPONENT_Accidential_Policy)) {
