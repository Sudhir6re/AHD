 ...
 if (!lstprop.isEmpty()) {
  ...
  lstObj.add(obj);
	...
	String HQL = "select sum(temp1.basic_pay)as basic_pay,sum(temp1.da)as da,sum(temp1.hra)as hra,sum(temp1.cla)as cla,sum(temp1.Income_tax)as Income_Tax,sum(temp1.hrr)as hrr,sum(temp1.pli)as pli, " + 
				...
				" sum(temp1.gpf_subscription)as gpf_subscription,sum(temp1.gpf_arrears)as gpf_arrears, sum(temp1.gpf_special_arr)as gpf_special_arr,sum(temp1.ndcps_subscription)as ndcps_subscription, sum(temp1.leave_pay)as leave_pay,sum(temp1.Mantralaya_Bank)as Mantralaya_Bank," + 
