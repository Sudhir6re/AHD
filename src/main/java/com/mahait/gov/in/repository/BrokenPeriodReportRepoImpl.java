...
public class BrokenPeriodReportRepoImpl implements BrokenPeriodReportRepo{
	...
	public List<DisplayBrokenPeriodReportModel> getAllDataForinnernew(String strddo, Integer month, Integer year,
		...
		String HQL = "select distinct COALESCE(d.department_allowdeduc_col_nm, d.department_allowdeduc_name) allded, d.is_type,d.department_allowdeduc_id,'0' tempvalue,' ' tempempty,department_allowdeduc_seq "
				...
				+ " ON e.broken_period_id=c.broken_period_id inner join department_allowdeduc_mst d ON b.department_allowdeduc_code=d.department_allowdeduc_code  where a.ddo_code='"+strddo+"' and billgroup_id="+billNumber+" and c.month_id="+month+" "
				+ " and c.year_id="+year+"   order by department_allowdeduc_seq  ";
	...
	public List<Map<String, Object>> getbrokenPeriodDetails(Integer month, Integer year, String strddo,
		...
		String HQL = "select b.sevaarth_id,a.employee_full_name_en,to_char(from_date, 'dd/MM/yyyy')as from_date,to_char(to_date, 'dd/MM/yyyy')as to_date,no_of_days,b.basic_pay,reason,remarks,b.net_pay,b.broken_period_id "
				+ " from employee_mst  a inner join broken_period_pay_mst b ON b.sevaarth_id=a.sevaarth_id  where month_id="+month+" and year_id="+year+" and a.ddo_code='"+strddo+"' and billgroup_id="+billNumber+" order by b.broken_period_id";
	...
	public List<Object[]> getbrokenPeriodvalue(BigInteger brokenid, String sevaarthId) {
		...
		String HQL =" select allow_deduc_code,allow_deduc_amt from broken_period_allow_deduc_mst a inner join  employee_allowdeduc_mpg b ON a.allow_deduc_code=b.department_allowdeduc_code where broken_period_id  ="+brokenid+" and b.sevaarth_id ='"+sevaarthId+"' ";
	...
	public Long findbillsize(int monthName, int yearName, Long billNumber, String ddoCode) {
		...
		query.append("select * from broken_period_pay_mst  where   year_id="+yearName+"  and  month_id ="+monthName+" and sevaarth_id in(select sevaarth_id from employee_mst  where ddo_code  ='"+ddoCode+"' and billgroup_id="+billNumber+")  ");
