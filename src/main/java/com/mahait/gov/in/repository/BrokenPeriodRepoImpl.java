...
//				mstEmployeeModel.setSevaarthId("0");
				...
				mstEmployeeModel.setDesignationName(getDesignationName(mstEmployeeEntity.getDesignationCode().toString()));
	...
	public List<Object[]> fetchAllowDeducName(String sevaarthid) {
		...
		String HQL = "select  COALESCE(deptallmt.department_allowdeduc_col_nm, deptallmt.department_allowdeduc_name) allded , deptallmt.is_type,deptallmt.department_allowdeduc_code,cgmst.group_name_en,cgmst.gis_amount  from  department_allowdeduc_mst deptallmt inner join employee_allowdeduc_mpg empalldecmpg on deptallmt.department_allowdeduc_code =  empalldecmpg.department_allowdeduc_code  inner join  employee_mst empmst on empmst.employee_id = empalldecmpg.employee_id inner join cadre_group_mst  cgmst    on empmst.emp_class = cgmst.id where empalldecmpg.sevaarth_id='"
				+ sevaarthid + "' and deptallmt.is_type in (1,2,4,3) order by  deptallmt.department_allowdeduc_code ";
	...
	@Override
	public List saveBrokenPeriodPay(BrokenPeriodEntity[] lArrMstBrokenPeriodPay, List<BrokenPeriodAllowDeducEntity> lListRltBrokenPeriodAllow,List<BrokenPeriodAllowDeducEntity> lListRltBrokenPeriodDeduc) {
...
//		 currentSession.createSQLQuery(HQL);.createSQLQuery(HQL);
	...
	public void deleteAllBrokenPeriodAllowancesForBrknPrdId(Long lLongBrokenPrdId)
	{
		...
		lSBQuery.append(" delete from BrokenPeriodAllowDeducEntity where brokenPeriodId = :brokenPeriodId and istype=1");
	...
	public void deleteAllBrokenPeriodDeductionsForBrknPrdId(Long lLongBrokenPrdId)
	{
		...
		lSBQuery.append(" delete from BrokenPeriodAllowDeducEntity where brokenPeriodId = :brokenPeriodId and istype=2");
