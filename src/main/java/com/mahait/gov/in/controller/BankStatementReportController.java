...
public class BankStatementReportController  extends BaseController {
	...
	@RequestMapping("/bankStatementReport")
	public String bankStatementReport(@ModelAttribute("bankStatementReportModel") BankStatementReportModel bankStatementReportModel,
			Model model, Locale locale, HttpSession session,HttpServletRequest request) {
		OrgUserMst messages = (OrgUserMst) session.getAttribute("MY_SESSION_MESSAGES");
		model.addAttribute("lstMonths", commonHomeMethodsService.lstGetAllMonths());
		model.addAttribute("lstYears", commonHomeMethodsService.lstGetAllYears());
		...
		Long billno=0l;
		String monname = null;
		List<Object[]>  billcreateornotbill = displayOuterReportService.findbillcrateornot(bankStatementReportModel.getMonthId(),bankStatementReportModel.getYearId(),messages.getDdoCode().toString(),
				bankStatementReportModel.getBillGroup().toString());
		...
		List<BankStatementReportModel>  bankstatementinfo = bankStatementReportService.findbankstatementinfo(bankStatementReportModel.getMonthId(),bankStatementReportModel.getYearId()
				,messages.getDdoCode(),String.valueOf(billno));
		  ...
		  double totalNetAmount = bankstatementinfo.stream().mapToDouble(BankStatementReportModel::getNetamt).sum();
			...
			BigInteger monthcurr = BigInteger.valueOf(bankStatementReportModel.getMonthId());
			BigInteger yearcurr = BigInteger.valueOf(bankStatementReportModel.getYearId());
			...
			String ddoname=null;
			Long ofcid=null;
			List<Object[]>  ddoinfo = paybillGenerationTrnService.findDDOinfo(messages.getDdoCode());
			...
			String billGrpname = displayInnerReportService.getbillDetails(billno);
			Date createdate = commonHomeMethodsService.findbillCreateDate(billno);
		...
		///	model.addAttribute("treasury",treasury);
			...
			model.addAttribute("yearName",bankStatementReportModel.getYearId());
			model.addAttribute("monthName",bankStatementReportModel.getMonthId());
			model.addAttribute("billNumber",billno);
			model.addAttribute("bankStatementReportModel", bankStatementReportModel);
