...
@Controller
@RequestMapping("/ddo")
public class NewRegDDOController {
	...
	public String viewFormsForwardedByAsstZpRepoDDO(@ModelAttribute("newRegDDOModel") NewRegDDOModel newRegDDOModel,
    	...
    	model.addAttribute("lstDDOOffice",newRegDDOService.findLvl1DDOCode(messages.getUserName()));	
...
*/		model.addAttribute("newRegDDOModel", newRegDDOModel);
	...
	public String approveEmployeeConfiguration(/*@ModelAttribute("mstEmployeeEntity") MstEmployeeEntity mstEmployeeEntity,*/
			Model model, Locale locale, HttpSession session,@PathVariable int employeeId) {/*
		String message = (String) model.asMap().get("message");
		...
		mstEmployeeEntity = empChangeDetailsService.getEmployeeData(employeeId);
...
//			payscalelevel = mstEmployeeService.findEmployeeConfigurationGetpayscale(mstEmployeeEntity.getPayCommissionCode().intValue());
	...
	public String viewFormsForwardedByAsst(@ModelAttribute("mstEmployeeModel") MstEmployeeModel mstEmployeeModel,
		...
		model.addAttribute("language", locale.getLanguage());
		String strddo = messages.getUserName();*/
	...
	public String loadFormListForDDO(@ModelAttribute("mstEmployeeModel") MstEmployeeModel mstEmployeeModel,
			Model model, Locale locale,
			HttpSession session) {
		String message = (String) model.asMap().get("message");
		model.addAttribute("mstEmployeeModel", mstEmployeeModel);
