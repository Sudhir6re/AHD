...
public class NSDLDetailsController  extends BaseController{
	...
	public ResponseEntity<InputStreamResource> validateNsdlFile(@ModelAttribute("nsdlDetailsModel") NSDLDetailsModel nsdlDetailsModel, Model model,
			Locale locale, HttpSession session, HttpServletRequest request)  {
	...
	public ResponseEntity<InputStreamResource> viewAndSaveFile(@PathVariable Integer month,@PathVariable Integer year,@PathVariable Integer treasuryId,@PathVariable String fileId,
			Locale locale, HttpSession session, HttpServletRequest request)  {
	...
	public String genNsdlTxtFile(HttpSession session,@ModelAttribute("nsdlDetailsModel") NSDLDetailsModel nsdlDetailsModel,Model model,Locale locale) {
		Long Num = null;
	...
	public String empContriDtls(@ModelAttribute("nsdlDetailsModel") NSDLDetailsModel nsdlDetailsModel, Model model,
			HttpSession session, Locale locale, @PathVariable String ddoCode,@PathVariable Integer month,@PathVariable Integer year) {
		OrgUserMst messages = (OrgUserMst) session.getAttribute("MY_SESSION_MESSAGES");
		String message = (String) model.asMap().get("message");
		...
		for (Object[] objddoLst : lstemployee1) {
			// String empName;
			empName = objddoLst[0].toString();
			 sevaarthId = objddoLst[2].toString();
			
			 gross = objddoLst[3].toString();
			net = objddoLst[4].toString();
			empAmt = objddoLst[5].toString();
			emprAmt = objddoLst[6].toString();
		}
		...
		model.addAttribute("empAmt", empAmt);*/
		model.addAttribute("lstNSDLDetailsModel", lstemployee1);
		model.addAttribute("nextMonth",nextMonth);
		NextYear="NPS Contribution of Month "+monname+" - " +curryear+" Paid In Month "+NextMon+"-"+NextYear;
