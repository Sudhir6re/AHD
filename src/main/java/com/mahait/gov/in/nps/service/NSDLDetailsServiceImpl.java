	...
	public List<DdoWiseNpsContriModel> searchDdoWiseContribution(int month, int year, OrgUserMst messages) {
		...
		if (!lstprop.isEmpty()) {
			for (Object[] objLst : lstprop) {
				...
				if(objLst[4]!=null) {
					ddoWiseNpsContriModel.setTotalEmpContri(StringHelperUtils.isNullBigDecimal(objLst[4]).longValue());
				}
				
				
				if(objLst[4]!=null && objLst[3]!=null) {
					ddoWiseNpsContriModel.setTotalEmpContri(ddoWiseNpsContriModel.getTotalEmpContri()+ddoWiseNpsContriModel.getTotalEmprContri());
				}
