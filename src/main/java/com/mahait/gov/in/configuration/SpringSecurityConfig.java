...
public class SpringSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().addFilterBefore(authenticationFilter(), UsernamePasswordAuthenticationFilter.class)
				...
				.antMatchers("/mdc/**").hasRole("MDC").antMatchers("/ddoast/**").hasRole("DDO_AST")
				.antMatchers("/ddo/**").hasRole("DDO").antMatchers("/super/**").hasRole("SUPER") // developer
				.antMatchers("/user/home").authenticated()
				.antMatchers("/services/**").denyAll()

				/* .anyRequest().authenticated() */
				.and().formLogin().loginPage("/user/login").permitAll()
				.successHandler(myAuthenticationSuccessHandler()).permitAll().and().logout().and().exceptionHandling()
				.accessDeniedPage("/user/login?unauthorize").and().logout().logoutUrl("/user/logOut")
				//.accessDeniedPage("/user/login?unauthorize").and().logout().logoutSuccessHandler(customLogoutSuccessHandler()) 
				.clearAuthentication(true).invalidateHttpSession(true).deleteCookies("JSESSIONID").and()
				.sessionManagement().invalidSessionUrl("/user/login?expired");
	}
