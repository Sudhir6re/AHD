		...
		$('body').on('click', '.empdata', function() {
			...
			$.ajax({
				...
				success : function(data) {
					console.log(data);
					 $("#appNo").empty();
					 $("#appNo").append('<option value="0">Please Select</option>');
					$("#loaderMainNew").hide();
					
					
					
					for (var i = 0; i < data.length; i++) {
						  $("#appNo").append( '<option  data-id="' + data[i].loanemployeeadvid + '"  value="' + data[i].loanaccountno + '">'+ data[i].loanaccountno + '</option>');
					}
				}
			});
			$("#searchDiv").hide();
					...
					success : function(data) {
						...
						for (var i = 0; i < data.length; i++) {
							 ...
							 // var principalAmt=data[i].id;
							  ...
							  var sanctionDate=new Date(data[i].loansancorderdate);
							  var voucherDate=new Date(data[i].voucherdate);
							  ...
							  var loanprininstno=data[i].loanprininstno;
								...
								$("#loanEmpAdvId").val(loanemployeeadvid);
							  
							  var day = ("0" + sanctionDate.getDate()).slice(-2);
							  var month = ("0" + (sanctionDate.getMonth() + 1)).slice(-2);

							  var today = sanctionDate.getFullYear()+"-"+(month)+"-"+(day) ;
							  
							  var day1 = ("0" + voucherDate.getDate()).slice(-2);
							  var month1 = ("0" + (voucherDate.getMonth() + 1)).slice(-2);

							  var today1 = voucherDate.getFullYear()+"-"+(month1)+"-"+(day1) ;
							  ...
							  $("#priRecovInsta").val(totalRecoveredInst);
